...
def build_graph(filenamelist, device=''):
    ...
    for file in filenamelist:
        ...
        for i in range(df.shape[0]):
            ...
            tmp_x = torch.tensor(dfi[1: ,:].astype('float32'))
            if parsed['useSeed']:
                tmp_x = torch.tensor(dfi[:,:].astype('float32'))
                tmp_x[0,1] = 0. ##set seed e to 0
            tmp_y = torch.tensor(dfi[0:1,:].astype('float32'))
...
class NN(object):
    def __init__(self, batch_size=64, gpu=0, smooth=0.05, parsed={}):
        ...
        self.model = CrystalNet(input_dim = 12 if self.parsed['notime'] else 13, ext_dim=11, output_dim = 1, grav_dim = 128, hidden_dim = 256, n_gravnet_blocks = 3, n_postgn_dense_blocks = 4, dropout = 0.1).to(self.device)
        self.loss = nn.L1Loss()
    ...
    def train(self, epoch):
        ...
        for i in idx:
            ...
            for grs in train_loader:
                ...
                z = self.model(grs.x[:,tmp_list], grs.batch, grs.y[:,2:])
                loss = self.loss(input=z.squeeze(), target=grs.y[:,1].squeeze())
                loss.backward()
