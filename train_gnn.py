...
class NN(object):
    def __init__(self, batch_size=64, gpu=0, smooth=0.05, parsed={}):
        ...
        input_dim = 8
        if self.parsed['notime']==True:
            input_dim = 7
        self.model = CrystalNet(input_dim = input_dim, ext_dim=6, output_dim = 1, grav_dim = 128, hidden_dim = 256, n_gravnet_blocks = 3, n_postgn_dense_blocks = 4, dropout = 0.1).to(self.device)
        self.loss = nn.L1Loss()
    ...
    def train(self, epoch):
        ...
        for i in idx:
            ...
            for grs in train_loader:
                ...
                z = self.model(grs.x, grs.batch, grs.y[:,2:8]) if len(tmp_list) == 0 else self.model(grs.x[:,tmp_list], grs.batch, grs.y[:,2:8])
