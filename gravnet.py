...
class CrystalNet(nn.Module):
    def __init__(self, input_dim: int = 20, ext_dim: int = 6, output_dim: int = 1, grav_dim: int = 64, hidden_dim: int = 256, n_gravnet_blocks: int = 3, n_postgn_dense_blocks: int = 3, dropout: float = 0, **kwargs):
        ...
        for i in range(self.n_postgn_dense_blocks):
            postgn_dense_modules.extend([
                nn.Linear(grav_dim*self.n_gravnet_blocks*4 + ext_dim if i == 0 else hidden_dim, hidden_dim),
                nn.Dropout(dropout),
                nn.ReLU(),
                nn.BatchNorm1d(hidden_dim),
            ])
