...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, filenamelist):
        ...
        for file in filenamelist:
            ...
            tmp_idx = np.logical_and(df[:,1,2,2] > parsed['E_min'], df[:,1,2,2] < parsed['E_max'])
            df = df[tmp_idx]
            if df.shape[0] <=0:continue
            df_sub = np.sum(df[:,1,:,:],axis=(1,2),keepdims=False)-df[:,1,2,2]
            tmp_idx = df_sub > 0
            df = df[tmp_idx]
            if df.shape[0] <=0:continue
            df[:,0,:,:] /= 40.
            ...
            df_sub  = np.copy(df)
            if parsed['useSeed']:
                df_sub[:,1,2,2] = 0. ##set seed e to 0
            else:
                df_sub[:,:,2,2] = 0. ##set seed e to 0
            tmp_label = torch.tensor(df_seed.astype('float32'))
            self.data_label = tmp_label if self.data_label is None else torch.cat((self.data_label,tmp_label),0)
            tmp_tensor = torch.tensor(df_sub.astype('float32'))
...
def count_training_evts(filenamelist):
    ...
    for file in filenamelist:
        ...
        label = f['feature']
        #tot_n += label.shape[0]
        tot_n += np.sum( np.logical_and(label[:,1,2,2] > parsed['E_min'],label[:,1,2,2] < parsed['E_max']) )
...
class NN(object):
    def __init__(self, batch_size=64, gpu=0, smooth=0.05, parsed={}):
        ...
        n_inputs = {'A0':256,'A1':6720,'A2':6272,'A3':2816,'A33':2688,'A':10752,'C':50176,'D':10752}
        if parsed['useRes']:
        ...
        hyperparameters = {
            ...
            'fcs_cfg':parsed['fcs'],
            'n_input':n_inputs[parsed['cfg'] ]+int(self.input_end-2),
        ...
        print('in_channels=',hyperparameters['in_channels'],',n_ext=',int(self.input_end-2))
    ...
    def train(self, epoch):
        ...
        for i in idx:
            ...
            for batch_idx, (da, da_label) in enumerate(train_loader):
                ...
                z = self.model(da[:,tmp_list,:,:], da_label[:,2:self.input_end])
                loss = self.loss(input=z.squeeze(), target=da_label[:,1].squeeze())
                loss.backward()
